<template>
  ...
  <SectionCard icon="mdi-shield-lock" title="네비게이션 가드 (Navigation Guard)">
    ...
    <!-- 복호화 로딩 중 -->
    <v-skeleton-loader v-if="isLoading" class="mb-4" type="card" />
    ...
    <v-card class="mt-4 pa-4" variant="outlined">
      ...
      <v-list density="compact">
        ...
        <v-list-item prepend-icon="mdi-numeric-3-circle">
          <code>AES-GCM 256bit</code>로 암호화하여 <code>localStorage</code>에 저장
          (Web Crypto API, 외부 패키지 없음)
        </v-list-item>
        <v-list-item prepend-icon="mdi-numeric-4-circle">
          암호화 키는 <code>sessionStorage</code>에 보관 → 탭 닫으면 키 소멸 → 자동 로그아웃
        </v-list-item>
      </v-list>
    </v-card>
  </SectionCard>
...
<script setup>
  ...
  const { isAuthenticated, isLoading, toggleAuth } = useAuth()
</script>
